...
/** @brief Fired when the OS/Windowing system has fully allocated resources for
 * the window. Vulkan surface creation is only safe after this event. */
 ...
 *  The window is NOT automatically destroyed; you need to call
 * lvkw_window_destroy() for that. */
...
typedef struct LVKW_WindowMaximizationEvent {
  bool maximized;  ///< True if the window is now maximized, false if it has
                   ///< been restored.
...
/** @brief Fired when the system's idle state changes based on the context's
 * idle_timeout. */
...
typedef struct LVKW_MonitorConnectionEvent {
  LVKW_Monitor *monitor;  ///< Information about the monitor that was connected
                          ///< or disconnected.
  bool connected;         ///< True if the monitor was connected, false if it was
                          ///< disconnected.
} LVKW_MonitorConnectionEvent;

/** @brief Fired when a monitor's mode (resolution/refresh rate) or scale
 * changes. */
...
/** @brief Fired when the user inputs text, potentially involving IMEs or
 * complex key combinations. This provides processed UTF-8 characters, whereas
 * LVKW_KeyboardEvent provides raw physical keys. */
...
typedef struct LVKW_TextCompositionEvent {
  const char *text;           ///< The current preedit string (UTF-8). Valid only during
                              ///< callback.
  uint32_t length;            ///< Length of the string in bytes (excluding null terminator).
  uint32_t cursor_index;      ///< Byte offset of the cursor within the preedit string.
  uint32_t selection_length;  ///< Byte length of the selection within the
                              ///< preedit string.
} LVKW_TextCompositionEvent;

/** @brief Fired when the window gains or loses focus from the OS. */
typedef struct LVKW_FocusEvent {
  bool focused;  ///< True if the window is now focused, false if it has lost
                 ///< focus.
} LVKW_FocusEvent;
...
typedef struct LVKW_DndFeedback {
  ...
  void **session_userdata;  ///< Persistent state for the duration of this DND
                            ///< session.
} LVKW_DndFeedback;
...
typedef struct LVKW_DndDropEvent {
  ...
  void **session_userdata;       ///< Access to the session state for final
                                 ///< processing/cleanup.
  const char **paths;            ///< Array of UTF-8 file paths. Valid only during the callback.
  LVKW_ModifierFlags modifiers;  ///< Active modifiers (Shift, Ctrl, etc.).
  uint16_t path_count;           ///< Number of file paths.
