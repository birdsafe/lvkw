...
typedef enum LVKW_MonitorFlags {
  LVKW_MONITOR_STATE_LOST = 1 << 0, ///< Monitor was disconnected. Handle is still valid but data is stale.
} LVKW_MonitorFlags;
 ...
 * @note **Lifetime:** Managed by the context. Handles remain valid until context destruction.
 *
 * ### Threading Model
 * Monitor operations follow the threading model of their parent @ref LVKW_Context.
 * Read-only access is thread-safe IF @ref LVKW_CTX_FLAG_PERMIT_CROSS_THREAD_API was set,
...
typedef struct LVKW_Monitor {
  ...
  LVKW_LogicalVec logical_position;  ///< READ ONLY: Top-left corner in global logical coordinates.
  LVKW_LogicalVec logical_size;      ///< READ ONLY: Size in logical coordinates (affected by @p scale).
  bool is_primary;                   ///< READ ONLY: True if this is the system's primary/default monitor.
 ...
 * @param[in,out] count Pointer to capacity (in) and actual number of active monitors (out).
 * @return LVKW_SUCCESS or LVKW_ERROR.
 */
LVKW_COLD LVKW_Status lvkw_ctx_getMonitors(LVKW_Context *ctx_handle, LVKW_Monitor **out_monitors, uint32_t *count);
 ...
 * @return LVKW_SUCCESS or LVKW_ERROR.
 */
LVKW_COLD LVKW_Status lvkw_ctx_getMonitorModes(LVKW_Context *ctx_handle, const LVKW_Monitor *monitor,
