...
int main(int, char **) {
  ...
  while (keep_going) {
    ...
    AppStatus status = app.update(ctx, window, io);
    if (status == AppStatus::EXIT) {
        keep_going = false;
    } else if (status == AppStatus::RECREATE_CONTEXT) {
        auto recreate_info = app.getRecreateInfo();
        ...
        {
            lvkw::Context old_ctx = std::move(ctx);
        }

        // Recreate context
        LVKW_ContextCreateInfo final_cci = LVKW_CONTEXT_CREATE_INFO_DEFAULT;
        final_cci.backend = recreate_info.backend;
        ...
        if (recreate_info.use_logging_allocator) {
            final_cci.allocator.alloc_cb = MetricsModule::logging_alloc;
            final_cci.allocator.realloc_cb = MetricsModule::logging_realloc;
            final_cci.allocator.free_cb = MetricsModule::logging_free;
        }
