...
```c
LVKW_Status lvkw_context_someFunction(LVKW_Context *ctx, ...) {
...
### Flow on Linux
1. **Selection**: During `lvkw_context_create`, LVKW probes for Wayland. If available, it loads the Wayland vtable; otherwise, it falls back to X11.
...
Functions that are truly cross-platform (like `lvkw_core_getVersion`) or provide shared logic (like `lvkw_wnd_getContext`) are implemented in `src/lvkw/base/lvkw_base.c`.
...
When adding a new function `lvkw_context_newMethod`:

1.  **Header**: Declare in `include/lvkw/lvkw-context.h`.
2.  **Validation**: Add `_lvkw_api_constraints_context_newMethod` in `src/lvkw/base/lvkw_api_constraints.h`.
3.  **VTable**: Add a function pointer to `LVKW_Backend` in `src/lvkw/base/lvkw_backend.h`.
4.  **Linux Dispatch**: Implement `lvkw_context_newMethod` in `src/lvkw/linux/lvkw_linux.c` using the vtable.
5.  **Backend Impls**:
    - Implement `lvkw_context_newMethod_WL` in `src/lvkw/linux/wayland/wayland_context.c`.
    - Implement `lvkw_context_newMethod_X11` in `src/lvkw/linux/x11/x11_context.c`.
    - Implement `lvkw_context_newMethod_Win32` (stub or impl) in `src/lvkw/win32/lvkw_win32.c`.
    - Implement `lvkw_context_newMethod_Mock` in `tests/mock_backend/mock_context.c`.
6.  **C++ Wrapper**: Add a corresponding wrapper to `lvkw::Context` or as a free function in `include/lvkw/lvkw.hpp` (and `include/lvkw/details/lvkw_hpp_impl.hpp`). If it involves C++20 features, use `include/lvkw/cpp/cxx20.hpp`.
