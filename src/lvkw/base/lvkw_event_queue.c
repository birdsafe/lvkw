...
LVKW_Status lvkw_event_queue_init(LVKW_Context_Base *ctx, LVKW_EventQueue *q, LVKW_EventTuning tuning) {
  ...
  if (tuning.initial_capacity > 0) {
    ...
    q->types = (LVKW_EventType *)lvkw_context_alloc_aligned(ctx, tuning.initial_capacity * sizeof(LVKW_EventType));
    q->windows = (LVKW_Window **)lvkw_context_alloc_aligned(ctx, tuning.initial_capacity * sizeof(LVKW_Window *));
    q->payloads = (LVKW_EventPayload *)lvkw_context_alloc_aligned(ctx, tuning.initial_capacity * sizeof(LVKW_EventPayload));
...
static bool _lvkw_event_queue_grow(LVKW_Context_Base *ctx, LVKW_EventQueue *q) {
  ...
  LVKW_EventType *new_types = (LVKW_EventType *)lvkw_context_alloc_aligned(ctx, new_capacity * sizeof(LVKW_EventType));
  LVKW_Window **new_windows = (LVKW_Window **)lvkw_context_alloc_aligned(ctx, new_capacity * sizeof(LVKW_Window *));
  LVKW_EventPayload *new_payloads =
      (LVKW_EventPayload *)lvkw_context_alloc_aligned(ctx, new_capacity * sizeof(LVKW_EventPayload));
...
bool lvkw_event_queue_push(LVKW_Context_Base *ctx, LVKW_EventQueue *q, LVKW_EventType type, LVKW_Window* window, const LVKW_Event *evt) {
  ...
  if (q->count >= q->capacity) {
    if (q->capacity >= q->max_capacity) {
      if (_is_event_compressible(type)) return false;
      ...
      q->count--;
      // Now q->tail (which was the old head) is effectively free as it's no longer in the [head, count] range
    }
    else {
  ...
  switch (type) {
    case LVKW_EVENT_TYPE_CLOSE_REQUESTED: q->payloads[q->tail].close_requested = evt->close_requested; break;
    case LVKW_EVENT_TYPE_WINDOW_RESIZED: q->payloads[q->tail].resized = evt->resized; break;
    case LVKW_EVENT_TYPE_KEY: q->payloads[q->tail].key = evt->key; break;
    case LVKW_EVENT_TYPE_WINDOW_READY: q->payloads[q->tail].window_ready = evt->window_ready; break;
    case LVKW_EVENT_TYPE_MOUSE_MOTION: q->payloads[q->tail].mouse_motion = evt->mouse_motion; break;
    case LVKW_EVENT_TYPE_MOUSE_BUTTON: q->payloads[q->tail].mouse_button = evt->mouse_button; break;
    case LVKW_EVENT_TYPE_MOUSE_SCROLL: q->payloads[q->tail].mouse_scroll = evt->mouse_scroll; break;
    case LVKW_EVENT_TYPE_IDLE_NOTIFICATION: q->payloads[q->tail].idle = evt->idle; break;
    case LVKW_EVENT_TYPE_MONITOR_CONNECTION: q->payloads[q->tail].monitor_connection = evt->monitor_connection; break;
    case LVKW_EVENT_TYPE_MONITOR_MODE: q->payloads[q->tail].monitor_mode = evt->monitor_mode; break;
    case LVKW_EVENT_TYPE_TEXT_INPUT: q->payloads[q->tail].text_input = evt->text_input; break;
    case LVKW_EVENT_TYPE_FOCUS: q->payloads[q->tail].focus = evt->focus; break;
    case LVKW_EVENT_TYPE_WINDOW_MAXIMIZED: q->payloads[q->tail].maximized = evt->maximized; break;
    case LVKW_EVENT_TYPE_DND_HOVER: q->payloads[q->tail].dnd_hover = evt->dnd_hover; break;
...
bool lvkw_event_queue_pop(LVKW_EventQueue *q, LVKW_EventType mask, LVKW_EventType* out_type, LVKW_Window** out_window, LVKW_Event *out_evt) {
  while (q->count > 0) {
    ...
    if (type & mask) {
      ...
      *out_window = q->windows[q->head];
      ...
      switch (type) {
        case LVKW_EVENT_TYPE_CLOSE_REQUESTED: out_evt->close_requested = q->payloads[q->head].close_requested; break;
        case LVKW_EVENT_TYPE_WINDOW_RESIZED: out_evt->resized = q->payloads[q->head].resized; break;
        case LVKW_EVENT_TYPE_KEY: out_evt->key = q->payloads[q->head].key; break;
        case LVKW_EVENT_TYPE_WINDOW_READY: out_evt->window_ready = q->payloads[q->head].window_ready; break;
        case LVKW_EVENT_TYPE_MOUSE_MOTION: out_evt->mouse_motion = q->payloads[q->head].mouse_motion; break;
        case LVKW_EVENT_TYPE_MOUSE_BUTTON: out_evt->mouse_button = q->payloads[q->head].mouse_button; break;
        case LVKW_EVENT_TYPE_MOUSE_SCROLL: out_evt->mouse_scroll = q->payloads[q->head].mouse_scroll; break;
        case LVKW_EVENT_TYPE_IDLE_NOTIFICATION: out_evt->idle = q->payloads[q->head].idle; break;
        case LVKW_EVENT_TYPE_MONITOR_CONNECTION: out_evt->monitor_connection = q->payloads[q->head].monitor_connection; break;
        case LVKW_EVENT_TYPE_MONITOR_MODE: out_evt->monitor_mode = q->payloads[q->head].monitor_mode; break;
        case LVKW_EVENT_TYPE_TEXT_INPUT: out_evt->text_input = q->payloads[q->head].text_input; break;
        case LVKW_EVENT_TYPE_FOCUS: out_evt->focus = q->payloads[q->head].focus; break;
        case LVKW_EVENT_TYPE_WINDOW_MAXIMIZED: out_evt->maximized = q->payloads[q->head].maximized; break;
        case LVKW_EVENT_TYPE_DND_HOVER: out_evt->dnd_hover = q->payloads[q->head].dnd_hover; break;
