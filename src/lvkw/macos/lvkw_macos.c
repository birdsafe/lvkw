...
LVKW_Status lvkw_createContext(const LVKW_ContextCreateInfo *create_info, LVKW_Context **out_ctx_handle) {
  ...
  if (create_info->backend != LVKW_BACKEND_AUTO && create_info->backend != LVKW_BACKEND_COCOA) {
    LVKW_REPORT_BOOTSTRAP_DIAGNOSTIC(create_info, LVKW_DIAGNOSTIC_BACKEND_UNAVAILABLE, "Requested backend is not supported on MacOS");
    return LVKW_ERROR;
  }
...
void lvkw_ctx_destroy(LVKW_Context *ctx_handle) {
  ...
  lvkw_ctx_destroy_Cocoa(ctx_handle);
}
...
const char *const *lvkw_ctx_getVkExtensions(LVKW_Context *ctx_handle, uint32_t *count) {
  ...
  return lvkw_ctx_getVkExtensions_Cocoa(ctx_handle, count);
}
...
void lvkw_wnd_destroy(LVKW_Window *window_handle) {
  ...
  lvkw_wnd_destroy_Cocoa(window_handle);
}
